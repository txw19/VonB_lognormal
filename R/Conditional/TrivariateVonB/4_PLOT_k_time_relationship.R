...
mean.beta <- exp(intro.pred.out$BUGSoutput$mean$BB[,2] )

# Fake data to predict
fake1 <- seq(min(z_estab), max(z_estab), length=20)

# # Obtain parameters of interest
# ests2 <- out2$BUGSoutput$summary[c("mu.k", "gammaKb1" ),1]
# # Fitted lines
...
for(i in 1:intro.pred.out$BUGSoutput$n.sims ){
  for(t in 1:length(fake1) ){
    est.lineA[i,t] <- exp(intro.pred.out$BUGSoutput$sims.list$mu.k[i] + intro.pred.out$BUGSoutput$sims.list$Kgamma1[i] * fake1[t] )
  }
}
...
for(i in 1:length(mean.beta)) { 
  u.alpha[i] <- exp(quantile(intro.pred.out$BUGSoutput$sims.list$BB[,i,2],probs=c(0.95) ) )
  l.alpha[i] <- exp(quantile(intro.pred.out$BUGSoutput$sims.list$BB[,i,2],probs=c(0.05) ) )
}

###########################################
####### FIGURE WITH CRI's
res <- 6
name_figure <- "K_established_tri.png"
